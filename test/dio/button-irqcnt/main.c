...
static int i2c_read(int fd, unsigned char cmd, unsigned char bnr, unsigned short *val) {
    ...
    messages[1].len   = 2;
    messages[1].buf   = (unsigned char *)val;
...
int main(void) {
	...
	/* enable irq for all but p1.3 */
	(void)i2c_write(fd, CMD_SET_PIR, 0xf7, 0);

	while(interrupted == 0) {

		/* read out counters */
		for(i = 0; i < 8; i++) {
			(void)i2c_read(fd, CMD_GET_PIRC, i, &btnc);
			printf("Button #%d=%04x\n", i, btnc);
		}
